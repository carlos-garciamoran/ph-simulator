<script lang="ts">
    ...
    import { phValueStore, checkedStore } from '@/utils/store'; // Ensure these paths are correct
    import { probePosition } from '@/utils/probePositionStore'; // Import the probe position store
...
<div class="border rounded-2xl w-full mx-auto p-6 h-fit flex flex-col gap-3">
    <div class="flex justify-between items-center">
        ...
        <span id="phValue" class="bg-secondary rounded-md text-2xl px-3 py-2 font-semibold">
            {$phValueStore.toFixed(2)} <!-- Use phValueStore here -->
        </span>
    </div>
